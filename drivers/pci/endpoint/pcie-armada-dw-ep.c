...
void armada_pcie_ep_write_header(void *ep_hdl, int func_id,
			      struct pci_epf_header *hdr)
{
	...
	writew_relaxed((hdr->baseclass_code << 8) | hdr->subclass_code,
			cfg_addr + PCI_CLASS_DEVICE);

	writew_relaxed(hdr->subsys_id,  cfg_addr + PCI_SUBSYSTEM_ID);
	writew_relaxed(hdr->subsys_vendor_id,
...
EXPORT_SYMBOL(armada_pcie_ep_write_header);
