...
scanagent_timing(int argc, char *argv[])
{
	...
	printf("%13s | %10s | %12s | %14s | %12s\n",
	       "Profile", "Dwell", "MaxOffChan", "MinLinkReturn", "RspTimeout");
	printf("-----------------------------------------------"
	       "--------------------------------\n");
	...
	while ((unsigned int)rsp_len > sizeof(MrvlIEtypesHeader_t)) {
		...
		case TLV_TYPE_SCAN_TIMING:
			...
			printf("%13s | %10d | %12d | %14d | %12d\n",
			       sel_str[le32_to_cpu(cfg_timing_tlv->mode) -
				       1].str,
...
{
	return (scanagent_table_entry_sub_cmd
		(argc, argv, SCAN_TABLE_OP_UNLOCK));
}
...
process_getscantable(int argc, char *argv[])
{
	...
	for (arg_idx = 0; arg_idx < argc; arg_idx++) {
		...
		} else if ((strncmp(argv[arg_idx], "held", strlen("held")) == 0)
			   || (strncmp(argv[arg_idx], "hold", strlen("hold")) ==
			       0)) {
			entryFlags |= SCAN_QUERY_ENTRY_FLAGS_HELD;
		} else if (strncmp(argv[arg_idx], "lock", strlen("lock")) == 0) {
			entryFlags |= SCAN_QUERY_ENTRY_FLAGS_LOCKED;
		} else if (strncmp(argv[arg_idx], "help", strlen("help")) == 0) {
			displayHelp = TRUE;
		} else if (strncmp(argv[arg_idx], "age=", strlen("age=")) == 0) {
			ageInSecs =
				strtol(argv[arg_idx] + strlen("age="), NULL,
				       10);
		} else if ((strncmp(argv[arg_idx], "ssid=", strlen("ssid=")) ==
			    0) && (num_ssid < NELEMENTS(ssidList))) {
		...
		} else if (strncmp(argv[arg_idx], "detail=", strlen("detail="))
			   == 0) {
			detail_idx =
				strtol(argv[arg_idx] + strlen("detail="), NULL,
				       10);
		}
	...
	if (detail_idx) {
		...
		if (num_ssid <= 1) {
			...
			/* Application filters for multiple SSIDs, don't pass down */
	...
	do {
		...
		for (idx = 0; idx < pRspInfo->scan_number; idx++) {
			...
			while (bss_info_length >= 2) {
				...
				case WPA_IE:
					pWpaIe = (IEEEtypes_VendorSpecific_t *)
						pElementId;
					if ((memcmp
					     (pWpaIe->vend_hdr.oui, wpa_oui,
					      sizeof(pWpaIe->vend_hdr.oui)) ==
					     0)
					    && (pWpaIe->vend_hdr.oui_type ==
						wpa_oui[3])) {
						/* Don't set privcap if WPA2/RSN already found */
						if (privCap != '2') {
				...
				case VHT_CAPABILITY:
					...
					break;
				case MDIE:
					/* Mobility domain IE.  Use to indicate 11r and MDID */
			...
			if (num_ssid) {
				/* Display filter based on potential multiple SSID input */
			...
			if (display) {
				 ...
				 */
				printf(" %c%c%c%c%c%c%c%c%c%c | ", capInfo.ibss ? 'A' : 'I', privCap,	/* P (WEP), W (WPA), 2 (WPA2) */
				       capInfo.spectrum_mgmt ? 'D' : ' ', wmmCap,	/* M (WMM), C (WMM-Call Admission Control) */
				       dot11kCap,	/* K */
				       pMdie ? 'R' : ' ',	/* 802.11r */
				       wps_cap,	/* S */
				       ht_cap,	/* N */
				...
				if (argc > 3 && strcmp(argv[3], "tsf") == 0) {
					...
					pByte = (t_u8 *)&fixed_fields.
						network_tsf;
					printf("    TSF=%02x%02x%02x%02x%02x%02x%02x%02x\n", pByte[7], pByte[6], pByte[5], pByte[4], pByte[3], pByte[2], pByte[1], pByte[0]);
				}
			}
		}
