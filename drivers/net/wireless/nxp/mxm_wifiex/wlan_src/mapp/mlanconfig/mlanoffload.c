		...
		case TLV_TYPE_MEASUREMENT_TIMING:
			...
			printf("%15s | %14d | %13d\n",
			       sel_str[le32_to_cpu(cfg_timing_tlv->mode) -
				       1].str,
			       (int)le32_to_cpu(cfg_timing_tlv->
						max_off_channel),
			       (int)le32_to_cpu(cfg_timing_tlv->
						max_on_channel));
			break;
...
process_assoc_timing(int argc, char *argv[])
{
	...
	cmdlen = sizeof(HostCmd_DS_GEN) +
		sizeof(HostCmd_DS_AssociationTiming_t);
...
process_link_stats(int argc, char *argv[])
{
	...
	pStats = (HostCmd_DS_LINK_STATS_SUMMARY *)(buf +
...
static void
execChanGroupCmd(t_u8 *cmdBuf, int tlv_size, t_u8 displayResult)
{
	...
	pChanGroup = (HostCmd_DS_OPCHAN_CHANGROUP_CONFIG *)(cmdBuf +
							    sizeof
							    (HostCmd_DS_GEN));
...
process_fcontrol(int argc, char *argv[])
{
	...
	pFcontrol = (HostCmd_DS_OFFLOAD_FEATURE_CONTROL *)(buf +
							   sizeof
							   (HostCmd_DS_GEN));
...
process_iapp(int argc, char *argv[])
{
	...
	fixlen = (sizeof(HostCmd_DS_GEN)
		  + sizeof(HostCmd_DS_IAPP_PROXY)
...
process_rf_tx_power(int argc, char *argv[])
{
	...
	pRfTxPower = (HostCmd_DS_802_11_RF_TX_POWER *)(buf +
						       sizeof(HostCmd_DS_GEN));
...
process_authenticate(int argc, char *argv[])
{
	...
	cmdlen = sizeof(HostCmd_DS_GEN) +
		sizeof(HostCmd_DS_802_11_AUTHENTICATE);
	buf = (t_u8 *)calloc(sizeof(t_u8), cmdlen);
	if (!buf) {
		printf("Memory allocation failed\n");
		return -ENOMEM;
...
process_chantrpcdisp(int startRate, int endRate)
{
	...
	totalTlvBytes = (le16_to_cpu(iwr.u.data.length)
			 - sizeof(HostCmd_DS_GEN)
			 - sizeof(pChanTrpc->action)
