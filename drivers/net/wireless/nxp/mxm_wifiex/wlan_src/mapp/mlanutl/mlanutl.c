...
process_getlog(int argc, char *argv[])
{
	...
	if (argc == 4 && !(strcmp(argv[3], "ext"))) {
		printf("rxStuckIssueCount-1                %u\n"
		       ...
		       stats->gdma_abort_cnt, stats->g_reset_rx_mac_cnt);
	}
	...
	if (action == MLAN_ACT_GET) {
		...
		case MLAN_11AXCMD_TXOPRTS_SUBID:
			printf("txop RTS Threshold: 0x%x\n",
			       axcmd->param.txop_cfg.
			       rts_thres & MASK_11AX_HE_RTS_THRESHOLD);
			break;
		case MLAN_11AXCMD_TXOMI_SUBID:
			printf("tx OMI: 0x%x\n",
			       axcmd->param.txomi_cfg.
			       omi & MASK_11AX_OM_CONTROL);
			break;
			  ...
			  wlan_ioctl_user_scan_cfg *scan_req)
{
	...
	const t_u8 auth_sha256_oui[4] = { 0x00, 0x0f, 0xac, 0x0B };	// 802.1x using SUITE-B, SHA-256
	const t_u8 auth_sha384_oui[4] = { 0x00, 0x0f, 0xac, 0x0C };	// 802.1x using SUITE-B, SHA-384
	const t_u8 auth_ft_sha384_oui[4] = { 0x00, 0x0f, 0xac, 0x0D };	// FT over 802.1x, SHA-384
		...
		for (idx = 0; (unsigned int)idx < prsp_info->scan_number; idx++) {
			...
			while (bss_info_length >= 2) {
				...
				case RSN_IE:
					...
					/* Get count for Auth Key Management TLVs */
					auth_key_mgmt_cnt = *pData;
					pData = pData + 2;

					/* Parse through all the AKM Suites to get desired OUI */

					while (auth_key_mgmt_cnt) {
						if (!memcmp
						    (pData, auth_sae_oui, 4)
						    || !memcmp(pData,
							       auth_sha256_oui,
							       4)
						    || !memcmp(pData,
...
process_tdls_setinfo(int argc, char *argv[])
{
	...
	while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
		...
		if (strcmp(args[0], "CapInfo") == 0) {
			...
			/* pairwise_cipher_suite + pairwise_offset + sizeof(t_u16) for akm_count */
			ptr = (t_u8 *)(rsn_ie->pairwise_cipher_suite +
			...
			/* pairwise_cipher_suite + pairwise_offset + sizeof(t_u16) for akm_count
			   + akm_offset + sizeof(t_u16) for RSNCapability + sizeof(t_u16) for PMKIDCount */
			ptr = (t_u8 *)(rsn_ie->pairwise_cipher_suite +
				       pairwise_offset + sizeof(t_u16))
...
static int
process_hal_phy_cfg(int argc, char *argv[])
{
	struct eth_priv_cmd *cmd;
	char *line = NULL;
	FILE *config_file = NULL;
...
static int
process_csi_cmd(int argc, char *argv[])
{
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd;
	struct ifreq ifr;
	...
	if (IS_HEX_OR_DIGIT(argv[3]) == MLAN_STATUS_FAILURE) {
		...
		memcpy(buffer + cmd_header_len + sizeof(csi_enable) +
		       4 * sizeof(t_u8), tailID, 4 * sizeof(t_u8));
		memcpy(buffer + cmd_header_len + sizeof(csi_enable) +
		       8 * sizeof(t_u8), (t_u8 *)&csi_filter_cnt,
		       sizeof(csi_filter_cnt));
		memcpy(buffer + cmd_header_len + sizeof(csi_enable) +
 ...
 */
static int
process_range_ext(int argc, char *argv[])
{
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd = NULL;
	struct ifreq ifr;
...
process_multi_chan_cfg(int argc, char *argv[])
{
	...
	if (argc == 3) {	/* GET operation */
		...
		if (multi_chan_info->tlv_len) {
			...
			if (tlv_buf_left <
			    (int)sizeof(MrvlIEtypes_multi_chan_info_t) ||
 ...
 */
static int
process_dfs53cfg(int argc, char *argv[])
{
	int ret = 0, data;
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd = NULL;
 ...
 */
static int
process_do_dfs_cac(int argc, char *argv[])
{
	int ret = 0;
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd;
...
/**
  ...
  */
static int
process_autodfs_cfg(int argc, char *argv[])
{
	struct eth_priv_cmd *cmd = NULL;
	struct ifreq ifr;
	...
	prepare_buffer(buffer, argv[2], 0, NULL);
	autodfs_cfg =
		(auto_zero_dfs_cfg *) (buffer + strlen(CMD_NXP) +
				       strlen(argv[2]));
...
receive_dot11_packet(char *ifName, int protocol, int verbose)
{
	...
	if (setsockopt
	    (sock_fd, SOL_SOCKET, SO_REUSEADDR, &sockopt,
	     sizeof sockopt) == -1) {
		perror("setsockopt");
		close(sock_fd);
		exit(EXIT_FAILURE);
	...
	/* Bind to device */
	if (setsockopt
	    (sock_fd, SOL_SOCKET, SO_BINDTODEVICE, ifName,
	     IFNAMSIZ - 1) == -1) {
 ...
 */
static int
process_dot11mc_unassoc_ftm_cfg(int argc, char *argv[])
{
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd = NULL;
	struct ifreq ifr;
 ...
 */
static int
process_arbcfg(int argc, char *argv[])
{
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd = NULL;
	struct ifreq ifr;
