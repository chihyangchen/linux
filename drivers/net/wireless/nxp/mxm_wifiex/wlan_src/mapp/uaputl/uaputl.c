...
__attribute__ ((format(printf, 2, 3)))
     static void uap_printf(int level, char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
...
static int
apcmd_beacon_stuck_detect(int argc, char *argv[])
{
	int opt;
...
apcmd_sys_config_profile(int argc, char *argv[])
{
	...
	while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
				...
				if (protocol != -1) {
					...
					if (protocol &
					    (PROTOCOL_WPA | PROTOCOL_WPA2 |
					     PROTOCOL_WPA3_SAE | PROTOCOL_OWE))
					{
						...
						tlv_akmp->tag =
							MRVL_AKMP_TLV_ID;
						tlv_akmp->length = 4;	/* sizeof(tlvbuf_akmp) - TLVHEADER */
						if (protocol &
						    PROTOCOL_WPA3_SAE) {
							tlv_akmp->key_mgmt =
								KEY_MGMT_SAE;
		...
		if (strcmp(args[0], "country") == 0) {
			...
			/* Send the command */
			if (uap_ioctl((t_u8 *)cmd_80211d_buf, &cmdlen, cmdlen)
			    == UAP_SUCCESS) {
...
print_bss_config(bss_config_t *buf)
{
	...
	if (buf->protocol & (PROTOCOL_WPA2 | PROTOCOL_WPA3_SAE | PROTOCOL_OWE)) {
		...
		if (argc == 2) {
			if (A2HEXDECIMAL(argv[1]) &
			    ~(KEY_MGMT_PSK | KEY_MGMT_PSK_SHA256 | KEY_MGMT_EAP
			      | KEY_MGMT_NONE | KEY_MGMT_OWE | KEY_MGMT_SAE)) {
