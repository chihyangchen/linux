...
#define CUS_EVT_CAC_FINISHED	"EVENT=CAC_FINISHED"
		if (!strncmp
		    ((char *)event, CUS_EVT_RADAR_DETECTED,
		     strlen(CUS_EVT_RADAR_DETECTED))) {
			printf("%s\n", (char *)event);
			break;
		}
		if (!strncmp
		    ((char *)event, CUS_EVT_CAC_FINISHED,
		     strlen(CUS_EVT_CAC_FINISHED))) {
			printf("%s\n", (char *)event);
			break;
		}
		if (!strncmp
		    ((char *)event, CUS_EVT_TOD_TOA, strlen(CUS_EVT_TOD_TOA))) {
		...
		if (!strncmp
			...
			t_u16 csi_length =
				0x1fff & ((event_mlan_csi *) event)->csi_record.
				Len;
