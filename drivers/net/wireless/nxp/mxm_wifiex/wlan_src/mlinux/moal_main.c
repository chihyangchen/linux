...
#ifdef CONFIG_RPS
static ssize_t
woal_set_rps_map(struct netdev_rx_queue *queue, const char *buf, size_t len)
{
...
#define MAX_WMM_QUEUE 4
	...
	if ((bss_type == MLAN_BSS_TYPE_DFS) && (dev_alloc_name(dev, name) < 0)) {
...
woal_init_priv(moal_private *priv, t_u8 wait_option)
{
	...
	if (priv->phandle->set_mac_addr && priv->bss_type != MLAN_BSS_TYPE_DFS) {
		memset(priv->current_addr, 0, ETH_ALEN);
	...
	for (index = 0; index < MAX_MLAN_ADAPTER; index++) {
		...
		for (i = 0; i < handle->priv_num; i++) {
...
#endif
					ieee80211_stop_queues(handle->priv[i]->
							      hw);
					ieee80211_unregister_hw(handle->
								priv[i]->hw);
					handle->priv[i]
						->mac80211_registered = MFALSE;
				}
				continue;
			}
#endif
			...
			if (handle->priv[i] &&
				...
				woal_11h_cancel_chan_report_ioctl(handle->
								  priv[i],
								  MOAL_IOCTL_WAIT);
				continue;
			}
