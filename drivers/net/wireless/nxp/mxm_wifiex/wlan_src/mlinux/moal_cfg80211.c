 ...
 */
int
woal_get_rx_freq(moal_private *priv, t_u8 band_config, t_u8 chan_num)
{
	int freq = 0;

	PRINTM(MEVENT, "band_config=%d, chan_num=%d\n", band_config, chan_num);
	if (chan_num) {
		return ieee80211_channel_to_frequency(chan_num
#if KERNEL_VERSION(2, 6, 39) <= CFG80211_VERSION_CODE
						      ,
						      (chan_num <=
						       14 ? IEEE80211_BAND_2GHZ
	...
	if (IS_STA_OR_UAP_CFG80211(priv->phandle->params.cfg80211_wext) &&
		...
		if (wiphy->bands[IEEE80211_BAND_2GHZ]) {
			if (((radio->param.ant_cfg.tx_antenna & 0xFF) != 3 &&
				...
				wiphy->bands[IEEE80211_BAND_2GHZ]->ht_cap.mcs.
					rx_mask[1] = 0;
		...
		if (wiphy->bands[IEEE80211_BAND_5GHZ]) {
			if (((radio->param.ant_cfg.tx_antenna & 0xFF00) != 0x300
				...
				(radio->param.ant_cfg.rx_antenna & 0xFF00) !=
				0)) {
				wiphy->bands[IEEE80211_BAND_5GHZ]->ht_cap.mcs.
					rx_mask[1] = 0;
				wiphy->bands[IEEE80211_BAND_5GHZ]->vht_cap.
					vht_mcs.rx_mcs_map =
 ...
 */
static void
woal_update_wiphy_chan_dfs_state(struct wiphy *wiphy, t_u8 channel,
				 t_u8 dfs_state)
{
 ...
 */
void
woal_update_channel_dfs_state(moal_private *priv, t_u8 channel, t_u8 dfs_state)
{
	...
	if (priv->phandle->pref_mac) {
		ref_handle = (moal_handle *)priv->phandle->pref_mac;
		wiphy = ref_handle->wiphy;
		if (!wiphy) {
			LEAVE();
			return;
		}
