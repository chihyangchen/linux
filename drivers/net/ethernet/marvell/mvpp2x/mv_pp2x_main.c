	 ...
	 struct mv_pp2x_tx_queue *txq, struct mv_pp2x_txq_pcpu *txq_pcpu)
{
	...
	for (i = 0; i < skb_shinfo(skb)->nr_frags; i++) {
		...
		/*Young 21_0616 change to bv_page*/
		//void *addr = page_address(frag->page.p) + frag->page_offset;
		void *addr = page_address(frag->bv_page) + frag->bv_offset;

		tx_desc = mv_pp2x_txq_next_desc_get(aggr_txq);
		tx_desc->phys_txq = txq->id;
		/*Young*/
		//tx_desc->data_size = frag->size;
		tx_desc->data_size = frag->bv_len;
			 ...
			 struct mv_pp2x_aggr_tx_queue *aggr_txq, int address_space)
{
	...
	if (frag_size == 0) {
		...
		frag_size = skb_frag_ptr->bv_len;
		frag_ptr = page_address(skb_frag_ptr->bv_page) + skb_frag_ptr->bv_offset;
		frag++;
	}
	total_desc_num = 0;
...
static inline bool mv_pp2x_skb_is_recycleable(const struct sk_buff *skb, int skb_size)
{
	if (unlikely(irqs_disabled()))
		return false;
	/*Young 21_0617 change the name to SKBFL_ZEROCOPY_ENABLE*/
...
static int mv_pp2x_phy_connect(struct mv_pp2x_port *port)
{
	...
	struct ethtool_link_ksettings *temp_link_mode;
	if (port->priv->pp2_version == PPV21)
	...
	temp_link_mode=(struct ethtool_link_ksettings *) kmalloc(sizeof(struct ethtool_link_ksettings),GFP_KERNEL);
	if(!temp_link_mode){
/*Young 21_0617*/
//	bitmap_and(phy_dev->supported,phy_dev->supported,PHY_GBIT_FEATURES,__ETHTOOL_LINK_MODE_MASK_NBITS);
//	linkmode_andnot(phy_dev->supported,phy_dev->supported,SUPPORTED_1000baseT_Half);	/* 1G-Half not supported */
		ethtool_convert_legacy_u32_to_link_mode(phy_dev->supported,SUPPORTED_1000baseT_Half);
		bitmap_complement(phy_dev->supported,phy_dev->supported,__ETHTOOL_LINK_MODE_MASK_NBITS);
		bitmap_and(phy_dev->supported,phy_dev->supported,PHY_GBIT_FEATURES,__ETHTOOL_LINK_MODE_MASK_NBITS);
...
//	phy_dev->advertising = phy_dev->supported;
	bitmap_copy(phy_dev->advertising,phy_dev->supported,__ETHTOOL_LINK_MODE_MASK_NBITS);
	}
	else{
		ethtool_convert_legacy_u32_to_link_mode(temp_link_mode->link_modes.supported,SUPPORTED_1000baseT_Half);
		bitmap_and(phy_dev->supported,phy_dev->supported,PHY_GBIT_FEATURES,__ETHTOOL_LINK_MODE_MASK_NBITS);
		linkmode_andnot(phy_dev->supported,phy_dev->supported,temp_link_mode->link_modes.supported);
		bitmap_copy(phy_dev->advertising,phy_dev->supported,__ETHTOOL_LINK_MODE_MASK_NBITS);
		kfree(temp_link_mode);
	}
			 			...
			 			struct net_device *sb_dev)
{
	...
	if (skb->queue_mapping)
		...
		return mv_pp2x_txq_number * fallback(dev, skb, sb_dev);
}
/*Young oringinal one below 21_0618 */
				 ...
				 struct device_node *emac_node)
{
	...
	//int phy_mode;
	u32 speed, id;
	/*Young 21_0618 modify the of_get_phy_mode*/
	...
	of_get_phy_mode(emac_node,&phy_mode);
			    ...
			    struct mv_pp2x *priv)
{
	...
	int port_num_irq;
	/*Young 21_0618 phy_interface_t & ret*/
	//int phy_mode;
	phy_interface_t phy_mode;
	int ret;
	...
	/* get MAC address */
	if (emac_node)
		ret = of_get_mac_address(emac_node,dt_mac_addr);
		//dt_mac_addr = of_get_mac_address(emac_node);

	//if (dt_mac_addr && is_valid_ether_addr(dt_mac_addr)) {
	if (!ret && is_valid_ether_addr(dt_mac_addr)) {
